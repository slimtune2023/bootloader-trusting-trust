...
static void data_abort_handler(regs_t *r) {
    ...
    printk("FAULT: PC = %x, Fault Address = %x, DFSR = %x\n", pc, fault_addr, dfsr);
    ...
    // if (fault_addr != test_addr)
    //     panic("Unexpected fault address! Expected %x, got %x\n", test_addr, fault_addr);
...
void notmain(void) {
    ...
    printk("Testing LOAD fault at %x\n", SEG_BCM_0);
    mmu_disable();
    pin_mmu_sec(4, SEG_BCM_0, SEG_BCM_0, no_access); 
    mmu_enable();
    lockdown_print_entries("");
    GET32(SEG_BCM_0);

    printk("Testing STORE fault at %x\n", SEG_BCM_0);
    mmu_disable();
    pin_mmu_sec(4, SEG_BCM_0, SEG_BCM_0, no_access); 
    mmu_enable();
    PUT32(SEG_BCM_0, 0xDEADBEEF);

    uint32_t *test_addr = kmalloc(4);
    *test_addr = 0xe12fff1e;
    printk("Testing JUMP fault at %x\n", test_addr);
    mmu_disable();
    pin_mmu_sec(1, SEG_HEAP, SEG_HEAP, no_access);
    mmu_enable();
    BRANCHTO((int)test_addr);
    printk("SUCCESS!!\n");
}
