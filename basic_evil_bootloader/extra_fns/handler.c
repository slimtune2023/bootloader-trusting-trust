...
static void data_abort_handler(regs_t *r) {
    ...
    if (fault_addr == AUX_STAT && !write) {
        ...
        *(char *)(RESERVED + 4 + ct) = (char)r->regs[1];
        ...
        // if (write) {
        //     PUT32(fault_addr, r->regs[1]);
        // } else {
        //     r->regs[0] = GET32(fault_addr);
        // }
        // printk("FAULT: PC = %x, Fault Address = %x, DFSR = %x, WRITE = %x\n", pc, fault_addr, dfsr, write);
        ;
    }
    ...
    //     // dev barrier
    //     dev_barrier();
    // } else if (fault_addr == 0x20200004 && write) {
    //     PUT32(0x20200004, r->regs[0]);
    // } else if (fault_addr == 0x20200004 && !write) {
    //     r->regs[0] = GET32(0x20200004);
    // } else if (fault_addr == 0x20215004 && write) {
    //     PUT32(0x20215004, r->regs[0]);
    // } else if (fault_addr == 0x20215004 && !write) {
    //     r->regs[0] = GET32(0x20215004);
    // } 
