...
static inline nrf_t *
server_mk(nrf_t *n, uint32_t rxaddr, uint32_t nbytes, int ack_p) {
    return nrf_init(n, server_conf(nbytes), rxaddr, ack_p);
}
static inline nrf_t *
server_mk_ack(nrf_t *n, uint32_t rxaddr, uint32_t nbytes) {
    return server_mk(n, rxaddr, nbytes, 1);
}
static inline nrf_t *
server_mk_noack(nrf_t *n, uint32_t rxaddr, uint32_t nbytes) {
    return server_mk(n, rxaddr, nbytes, 0);
}

// same as server, but for clients.
static inline nrf_t *
client_mk(nrf_t *n, uint32_t rxaddr, uint32_t nbytes, int ack_p) {
    return nrf_init(n, client_conf(nbytes), rxaddr, ack_p);
}
static inline nrf_t *
client_mk_ack(nrf_t *n, uint32_t rxaddr, uint32_t nbytes) {
    return client_mk(n, rxaddr, nbytes, 1);
}
static inline nrf_t *
client_mk_noack(nrf_t *n, uint32_t rxaddr, uint32_t nbytes) {
    return client_mk(n, rxaddr, nbytes, 0);
}
#endif
